        ...
        <div class="row">
            <div class="col-12">
                <ul class="list-unstyled mb-0">
                    ...
                    <li class="d-inline-block font-weight-bolder">Cart</li>
                </ul>
            </div>
        </div>
    </div>
                                ...
                                <div class="row align-items-center">
                                    <div class="col-3">
                                        <div class="item-image">
                                            <img src="{{ $item->product->image ? asset('storage/' . $item->product->image) : asset('assets/images/product/product1.jpg') }}" alt="{{ $item->product->name }}">
                                        </div>
                                    </div>
                                    <div class="col-9">
                                        <div class="item-details">
                                            ...
                                            <div class="item-quantity d-flex align-items-center mt-2">
                                                <form action="{{ route('cart.update', $item) }}" method="POST" class="d-inline me-3">
                                                    @csrf
                                                    <div class="d-flex align-items-center">
                                                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="this.parentNode.querySelector('input').stepDown(); this.form.submit()">−</button>
                                                        <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" class="form-control mx-2 text-center" style="width:60px;">
                                                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="this.parentNode.querySelector('input').stepUp(); this.form.submit()">+</button>
                                                    </div>
                                                </form>
                ...
                <div class="col-lg-4">
                    <div class="summary-card p-4 border rounded bg-light">
                        ...
                        <div class="summary-row d-flex justify-content-between">
                            <span>Subtotal:</span>
                            <span class="summary-amount">₹{{ number_format($subtotal, 2) }}</span>
                        </div>
                        <div class="summary-row d-flex justify-content-between">
                            <span>Shipping:</span>
                        ...
                        <div class="summary-row total d-flex justify-content-between mt-3 pt-3 border-top">
                            <span>Total:</span>
                            <span class="summary-amount total">₹{{ number_format($total, 2) }}</span>
                        </div>
                        <a href="{{ route('checkout') }}" class="checkout-btn btn btn-success w-100 mt-3">Proceed to Checkout</a>
