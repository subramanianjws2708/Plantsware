            ...
            <div class="row mb-3 align-items-center">
                ...
                <div class="col-md-6 text-center">
                    <div class="d-inline-flex flex-wrap" role="group">
                        @php
                            $statuses = [
                                '' => ['label' => 'All', 'class' => 'btn-light'],
                                'pending' => ['label' => 'Pending', 'class' => 'btn-warning'],
                                'processing' => ['label' => 'Processing', 'class' => 'btn-info'],
                                'shipped' => ['label' => 'Shipped', 'class' => 'btn-light-primary'],
                                'delivered' => ['label' => 'Delivered', 'class' => 'btn-light-success'],
                                'cancelled' => ['label' => 'Cancelled', 'class' => 'btn-light-danger'],
                                'returned' => ['label' => 'Returned', 'class' => 'btn-light-secondary'],
                            ];
                        @endphp
                        @foreach($statuses as $code => $meta)
                            <a href="{{ route('admin.orders.index', array_merge(request()->except('page'), $code ? ['status' => $code] : ['status' => null])) }}"
                               class="btn btn-sm {{ $meta['class'] }}{{ request('status') === $code || (empty(request('status')) && $code === '') ? ' active' : '' }} me-1 mb-1">
                                {{ $meta['label'] }}
                            </a>
                        @endforeach
                    </div>
