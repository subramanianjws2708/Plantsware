...
@php
    // Try to get the $category from the first subcategory, or from request
    if (isset($subcategories) && count($subcategories)) {
        $category = $subcategories->first()->category ?? null;
    } else {
...
@section('title', 'Subcategories' . (isset($category) && $category ? ' - ' . $category->name : ''))
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2>
        Subcategories{{ isset($category) && $category ? ' in &quot;' . $category->name . '&quot;' : '' }}
    </h2>
